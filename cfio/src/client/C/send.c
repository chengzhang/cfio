...
static inline int _send_msg(cfio_msg_t *msg)
{
    MPI_Status status;
...
//	    msg->dst, tag, msg->comm, &(msg->req));
...
#ifdef disable_merge
    // if merge disabled, send msg immediately
    #ifdef async_isend
    MPI_Isend(msg->addr, msg->size, MPI_BYTE, 
	    msg->dst, msg->src, msg->comm, &(msg->req));
    qlist_add_tail(&(msg->link), &(msg_head->link));
    ...
    #endif // async_isend
#else
    // if merge enabled, merge msg to merge_msg firstly, if cannot merge, send msg immediately

    if ((msg->func_code == FUNC_FINAL) || (msg->func_code ==  FUNC_IO_END)
	    || (msg->func_code == FUNC_NC_PUT_VARA)) { 
	//FINAL,  IO_END, not merge
